...
class nameViewController: UIViewController,UITextFieldDelegate {
    ...
    func registerPhoneNo() {
        ...
        ApiHandler.sharedInstance.registerPhone(phone: phoneNo, dob: dob, username: username, country_id: self.country_id) { (isSuccess, response) in
            if isSuccess{
...
//                        NotificationCenter.default.post(name: Notification.Name("dismissVC"), object: nil)
    ...
    func registerSocialUser() {
        ...
        ApiHandler.sharedInstance.registerSocialUser(dob: self.dob, country_id: self.country_id, username: username, email: self.socialEmail, social_id: self.socialID, social: signUPType!, first_name: self.firstName, last_name: self.lastName, auth_token: self.authToken, device_token: deviceToken!) { (isSuccess, response) in
            if isSuccess{
                if response?.value(forKey: "code") as! NSNumber == 200 {
